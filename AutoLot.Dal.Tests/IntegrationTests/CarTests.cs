...
namespace AutoLot.Dal.Tests.IntegrationTests
{
    ...
    public class CarTests : BaseTest, IClassFixture<EnsureAutoLotDatabaseTestFixture>
    {
        ...
        {
            IIncludableQueryable<Car, Customer?> query = Context.Cars.Where(c => c.Orders.Any()).Include(c => c.MakeNavigation).Include(c => c.Orders).ThenInclude(o => o.CustomerNavigation);
            var qs = query.ToQueryString();
            OutputHelper.WriteLine($"Query: {qs}");
            var cars = query.ToList();
        ...
        public void ShouldGetCarsOnOrderWithRelatedPropertiesAsSplitQuery()
        {
            ...
            .Include(c => c.MakeNavigation)
            .Include(c => c.Orders).ThenInclude(o => o.CustomerNavigation).AsSplitQuery();
            var qs = query.ToQueryString();
            OutputHelper.WriteLine($"Query: {qs}");
            var cars = query.ToList();
            Assert.Equal(5, cars.Count);
        ...
        public void ShouldNotGetTheLemonsUsingFromSql()
        {
            ...
            var query = Context.Cars.FromSqlRaw("Select *, ValidFrom, ValidTo from {0}.{1}", schemaName, tableName);
            var qs = query.ToQueryString();
            OutputHelper.WriteLine($"Query: {qs}");
            var cars = query.ToList();
            Assert.Equal(9, cars.Count);
...
.Include(x => x.MakeNavigation);
        ...
        {
            Assert.Equal(expectedName, _carRepo.GetNickName(id));
        }
...
{
new() { Color = "Yellow", MakeId = 1, NickName = "Herbie" },
new() { Color = "White", MakeId = 2, NickName = "Mach 5" },
new() { Color = "Pink", MakeId = 3, NickName = "Avon" },
new() { Color = "Blue", MakeId = 4, NickName = "Blueberry" },
};
                var carCount = Context.Cars.Count();
        ...
        public void ShouldUpdateACar()
        {
            ...
            void RunTheTest(IDbContextTransaction trans)
            {
                ...
                var context2 = TestHelper.GetSecondContext(Context, trans);
        ...
        public void ShouldUpdateACarUsingState()
        {
            ...
            void RunTheTest(IDbContextTransaction trans)
            {
                ...
                var context3 =
                TestHelper.GetSecondContext(Context, trans);

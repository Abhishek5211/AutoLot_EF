...
namespace AutoLot.Dal.EfStructures
{
    ...
    public class ApplicationDbContext : DbContext
    {
        ...
        private void ChangeTracker_Tracked(object sender, EntityTrackedEventArgs e)
        {
            ...
            {
                Console.WriteLine($"Car entry {c.NickName} was added to {source}");
            }
        }
        ...
        private void ChangeTracker_StateChanged(object sender, EntityStateChangedEventArgs e)
        {
            ...
            Console.WriteLine($"The Car was changed from {e.OldState} to {e.NewState}");
        ...
        public ApplicationDbContext(DbContextOptions options) : base(options)
        {
            SavingChanges += (sender, args) =>
            {
                ...
                Console.WriteLine($"The Changes are currently saving for {cs}");
            };

            SavedChanges += (sender, args) =>
            {
                string cs = ((ApplicationDbContext)sender).Database!.GetConnectionString();
                Console.WriteLine($"The Changes are Saved for {cs}");
            };
            SaveChangesFailed += (sender, args) =>
            {
                string cs = ((ApplicationDbContext)sender).Database!.GetConnectionString();
                Console.WriteLine($"Saving Changes failed for {cs}. Exception: {args.Exception.Message}");
            };
